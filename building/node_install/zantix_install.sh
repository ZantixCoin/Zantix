...
echo -e "${GREEN}Refreshing available system packages and updates.${NC}"
DEBIAN_FRONTEND=noninteractive apt-get update > /dev/null 2>&1
echo -e "${GREEN}Updating system.${NC}"
DEBIAN_FRONTEND=noninteractive apt-get -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y -qq upgrade >/dev/null 2>&1
apt install -y software-properties-common >/dev/null 2>&1
echo -e "${GREEN}Adding the bitcoin repository.${NC}"
apt-add-repository -y ppa:bitcoin/bitcoin >/dev/null 2>&1
echo -e "${GREEN}Installing required additional packages, this may take some time to finish depending on the system specs. ${RED}Don't press any keys.${NC}"
apt-get update >/dev/null 2>&1

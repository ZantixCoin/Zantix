...
Zantix now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<zantix_address\> \<monthly_payment_zantix\> [use_ix(true|false)]

Example:
```
mnbudget prepare cool-project http://www.cool-project/one.json 12 100000 Z6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7 1200 true
```
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ ZNTX, _12_ times over the course of a year totaling _24000_ ZNTX.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<zantix_address\> \<monthly_payment_zantix\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` ZNTX.
 ...
 * vote-many          - Vote on a Zantix initiative
 * vote-alias         - Vote on a Zantix initiative
 * vote               - Vote on a Zantix initiative/budget
